...
import { v3PositionManagerAbi, v4PoolManagerAbi, v4StateViewAbi, type DeploymentAddresses } from '../contracts'
import type { PositionCashflowWrite, StateDatabase } from '../state/database'
import type { PortfolioPriceOracle } from './price-oracle'
import { getAmountsForLiquidity, tokenIdSalt, type PositionData } from './positions'
...
async function getEventsChunked(client: PublicClient, input: Parameters<PublicClient['getContractEvents']>[0]) {
  ...
  // viem's generic event type cannot be retained across this ABI-agnostic
  // chunking helper; each caller supplies and narrows its concrete ABI.
